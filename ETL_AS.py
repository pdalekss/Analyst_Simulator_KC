...
def dag_table_by_group():
    ...
    query_feed = """
                    ...
                    GROUP BY day, user_id, gender, age, os
                 """
    query_message = """
                    SELECT 
                      l.day as day,
...
"""
    query_create_table = """
            CREATE TABLE IF NOT EXISTS test.pda_etl_as (
                event_date Date,
    ...
    def transform_group(df_cube, group):
        ...
        df = df \
            ...
            .agg({'likes': 'sum',
                  ...
                  'users_received': 'sum',
                  'users_sent': 'sum'}) \
            .rename(columns={f'{group}': 'dimension_value', 'day': 'event_date'})

        df['dimension'] = group
        df = df[['event_date', 'dimension', 'dimension_value',
                 'likes', 'views', 'messages_sent', 'messages_received',
    ...
    def load(query_create_table, connection_test, df):
        ...
        ph.to_clickhouse(df, 'pda_etl_as', index=False, connection=connection_test)
