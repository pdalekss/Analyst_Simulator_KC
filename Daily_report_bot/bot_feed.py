...
def dag_daily_report_feed():
    ...
    def get_data(query, connection):
        ...
        return df
    @task
    def get_report(yesterday_metrics, last_week_metrics):
        def get_plot(last_week_metrics):
            ...
            plt.xticks(rotation=45)
            fig.suptitle('Ключевые метрики за предыдущие 7 дней', fontweight="bold", fontsize=18)

            plot_object = io.BytesIO()
            plt.savefig(plot_object)
            plot_object.seek(0)
            plot_object.name = 'previos_week_feed_report.png'
            plt.close()
    ...
    #EXTRACT
    yesterday_metrics = get_data(query=query_yesterday, connection=connection_simulator_20221120)
    last_week_metrics = get_data(query=query_last_week, connection=connection_simulator_20221120)
    #TRANSFORM
    rep = get_report(yesterday_metrics, last_week_metrics)
    #LOAD
